...
func TestHandlerServeOpenAPISpec(t *testing.T) {
	t.Run("rewrites servers strips auth and adds priority header", testHandlerServeOpenAPISpecRewritesServers)
	t.Run("does not duplicate priority header parameter", testHandlerServeOpenAPISpecNoDuplicatePriority)
	t.Run("simplifies info description", testHandlerServeOpenAPISpecSimplifiesInfo)
	t.Run("returns bad gateway on upstream errors", testHandlerServeOpenAPISpecBadGateway)
}

func testHandlerServeOpenAPISpecRewritesServers(t *testing.T) {
...
func testHandlerServeOpenAPISpecNoDuplicatePriority(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
				"openapi":"3.0.0",
...
func testHandlerServeOpenAPISpecSimplifiesInfo(t *testing.T) {
...
func testHandlerServeOpenAPISpecBadGateway(t *testing.T) {
	tests := []struct {
		name    string
		specURL string
